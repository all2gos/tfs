...
if len(selected_images) != num_companions:
    ...
    your_lvl = int(st.text_input('Please enter your lvl:'))
    ...
    if type_of_fight == 'map/dung':
        your_s = int(st.text_input('Please enter your strenght in dun/map:'))
        your_d = int(st.text_input('Please enter your dexternity in dun/map:'))
        your_c = int(st.text_input('Please enter your constitution in dun/map:'))
        your_i = int(st.text_input('Please enter your intelligence in dun/map:'))

        st.write('Also we need your main character stats separately')

        char_s = int(st.text_input('Please enter char strenght:'))
        char_d = int(st.text_input('Please enter char dexternity:'))
        char_c = int(st.text_input('Please enter char constitution:'))
        char_i = int(st.text_input('Please enter char intelligence:'))
        ...
        your_dmg_min, your_dmg_max = [int(x) for x in your_dmg.split('-')]
        ...
        your_armor = int(your_armor)

        your_poey = 0
        if st.checkbox('Select this if you use Potion of Eternal Youth'):
            your_poey = 1
        rune_lvl = int(st.text_input('Please provide your Ruby level'))/1000
        ...
        stat_potion_inf = 1+ 0.1*(1+rune_lvl) if potion_size == 'small' else 1+ 0.15*(1+rune_lvl) if potion_size == 'medium' else 1 + 0.25*(1+rune_lvl)

        st.markdown('#### Enemy info')  
        enemy_hp = 0
        enemy_lvl = int(st.text_input('Please enter enemy level:'))
        enemy_s = int(st.text_input('Please enter enemy strenght:'))
        enemy_d = int(st.text_input('Please enter enemy dexternity:'))
        enemy_c = int(st.text_input('Please enter enemy constitution:'))
        if st.checkbox('Click here if you rather would like to type enemy hp by hand (recommended)'):
            enemy_hp = int(st.text_input('Please enter enemy hp'))
        enemy_i = int(st.text_input('Please enter enemy intelligence:'))

        enemy_dmg = st.text_input('Add enemy weapon damage in form 50-100 (Based on the visual look of the weapon, usually 1-2)')
        enemy_dmg_min, enemy_dmg_max = [int(x) for x in enemy_dmg.split('-')]
